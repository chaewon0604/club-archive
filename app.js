...
function renderFile(path) {
  ...
  // PDF: PDF.js 뷰어로 열기 (스탠바이미에서 새 탭 튐 방지)
if (isPdf(path)) {
  // path가 "assets/pdfs/xxx.pdf" 같은 상대경로여도 절대경로로 바꿔서 viewer에 전달
  const absPdfUrl = new URL(path, window.location.href).href;

  // PDF.js viewer.html로 열기
  const viewerUrl = `assets/pdfjs/web/viewer.html?file=${encodeURIComponent(absPdfUrl)}`;
